...
def main():
    ...
    # Main control loop
    episode_duration = 1 # duration in seconds
    current_time = 0
    ...
    for i in range(steps):
        ...
        for j in range(N_mpc):
            ...
            #This is only control position:
            #q_d, _ = ref.get_values(current_time + j*time_step) 
        ...
        #cmd.tau_cmd = feedback_lin_ctrl(dyn_model, q_mes, qd_mes, u_mpc)
        ctrl_cmd = dyn_cancel(dyn_model, q_mes, qd_mes, u_mpc)
        cmd.SetControlCmd(ctrl_cmd,["torque"]*7)
